...
export async function shipCommand(options: ShipOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (currentBranch === defaultBranch) {
      logger.error(
        `Cannot ship from ${defaultBranch} branch. Create a feature branch first.`,
      );
      process.exit(1);
    }
    ...
    if (!options.skipSecurity) {
      ...
      if (!securityResult.passed) {
        ...
        securityResult.blockers.forEach((blocker) =>
          logger.log(`  • ${blocker}`),
        );
        ...
        logger.log(
          `  • ${chalk.cyan("gpm security")} - View detailed security report`,
        );
    ...
    if (waitForChecks && !options.skipCi) {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (!options.skipAutoFix && result.summary.failed > 0) {
            ...
            for (const failure of result.summary.failureDetails) {
              logger.info(
                `Analyzing: ${chalk.cyan(failure.checkName)} (${failure.errorType})`,
              );

              const fixResult = await autoFixService.attemptFix(
                failure,
                prNumber,
              );
              ...
              if (fixResult.success) {
                ...
                logger.warn(
                  `⚠️  Cannot auto-fix: ${failure.checkName} (${reason})`,
                );
              }
            }
        ...
        try {
          (tracker as ExecutionTracker).logFailed("unknown", error.message);
        } catch {
