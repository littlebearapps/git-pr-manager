...
function main() {
  ...
  const tools = [
    ...
    },
    {
      name: "detect-secrets",
      required: false,
      purpose: "Secret scanning (pip install detect-secrets)",
    },
    {
      name: "pip-audit",
      required: false,
  ...
  if (missing.length > 0) {
    ...
    if (requiredMissing.length > 0) {
      console.log(
        `❌ Required tools missing: ${requiredMissing.map((t) => t.name).join(", ")}`,
      );
    ...
    if (optionalMissing.length > 0) {
      ...
      console.log(
        "   gpm will skip these scans gracefully when tools are unavailable\n",
      );
    }
  }
