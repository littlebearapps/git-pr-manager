...
program
  .name("gpm")
  .description(
    "Git PR Manager - Enhanced git workflow automation with CI integration",
  )
  .version(getVersion())
...
program
  ...
  .option(
    "--title <title>",
    "PR title (auto-generated from branch if not provided)",
  )
...
program
  ...
  .option(
    "--template <type>",
    "Configuration template (basic, standard, strict)",
    "basic",
  )
...
program
  ...
  .option(
    "--preset <type>",
    "Protection preset (basic, standard, strict)",
    "standard",
  )
...
program
  ...
  .option(
    "--allow-install",
    "Allow automatic dependency installation (Phase 1b)",
  )
...
program
  ...
  .option(
    "--guide <name>",
    "Show specific guide (AI-AGENT-INTEGRATION, GITHUB-ACTIONS-INTEGRATION, etc.)",
  )
  .action(docsCommand);
