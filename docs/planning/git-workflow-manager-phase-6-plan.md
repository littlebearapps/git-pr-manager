...
Phase 6 will transform git-workflow-manager from a passive error reporter into an **active error remediation system** by adding execution capabilities to the existing SuggestionEngine. This phase focuses on **deterministic auto-fixes** using proven tools (eslint --fix, prettier, black, etc.) without AI/LLM integration.
...
Based on empirical data from git-workflow-manager usage:
...
**Configuration**:
- Optional: Add `autoFix` section to `.gwm.yml`
