...
describe('ExecutionTracker', () => {
  ...
  it('logs skipped steps with reason', () => {
    ...
    const { steps } = tracker.getSummary();
    expect(steps[0]).toEqual({ name: 'verification', status: 'skipped', reason: '--skip-verify flag' });
  });
  ...
  it('logs failed steps with reason', () => {
    ...
    const { steps } = tracker.getSummary();
    expect(steps[0]).toEqual({ name: 'unknown', status: 'failed', reason: 'unexpected error' });
  });
  ...
  it('preserves step order', () => {
    ...
    const { steps } = tracker.getSummary();
    expect(steps.map(s => s.name)).toEqual(['verification', 'security', 'push']);
  });
