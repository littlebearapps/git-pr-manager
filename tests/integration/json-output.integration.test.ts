...
describe('JSON output contract', () => {
  ...
  it('status emits exactly one JSON object + newline', async () => {
    const mockGit = {
      getBranchInfo: jest.fn().mockResolvedValue({ current: 'feature/test', isClean: true }),
      getStatus: jest.fn().mockResolvedValue({ modified: [], created: [], deleted: [], not_added: [] }),
    } as any;
    ...
    (ConfigService as unknown as jest.Mock).mockImplementation(() => mockConfig);
