...
describe('ConfigService', () => {
  ...
  describe('load', () => {
    ...
    it('should load and parse config file', async () => {
      ...
      expect(config.security?.allowedVulnerabilities).toEqual(['CVE-2023-1234']);
    });
    ...
    it('should throw error on parse failure', async () => {
      ...
      await expect(configService.load()).rejects.toThrow('Failed to load config');
    });
  ...
  describe('save', () => {
    ...
    it('should throw error on write failure', async () => {
      ...
      await expect(configService.save({} as any)).rejects.toThrow('Failed to save config');
    });
  });
  ...
  describe('init', () => {
    it('should throw error if config already exists', async () => {
      ...
      await expect(configService.init()).rejects.toThrow('Config file already exists');
    });
