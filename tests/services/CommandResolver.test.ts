...
describe('CommandResolver', () => {
  ...
  describe('constructor', () => {
    ...
    it('should initialize with default working directory', () => {
      jest.clearAllMocks();
      new CommandResolver();
      expect(MockedLanguageDetectionService).toHaveBeenCalledWith(process.cwd());
    });
  });
  ...
  describe('resolve', () => {
    ...
    it('should use package manager command when available', async () => {
      ...
      expect(mockLanguageDetector.checkToolAvailable).toHaveBeenCalledWith('npm');
    });
  ...
  describe('getSuggestedInstallCommand', () => {
    ...
    it('should return install command for pytest (Python)', () => {
      const suggestion = resolver.getSuggestedInstallCommand('pytest', 'python');
    ...
    it('should return install command for eslint (Node.js)', () => {
      const suggestion = resolver.getSuggestedInstallCommand('eslint', 'nodejs');
      expect(suggestion).toBe('npm install -D eslint');
    });

    it('should return install command for golangci-lint (Go)', () => {
      const suggestion = resolver.getSuggestedInstallCommand('golangci-lint', 'go');
      expect(suggestion).toBe('go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest');
    });

    it('should return null for unknown tool', () => {
      const suggestion = resolver.getSuggestedInstallCommand('unknown-tool', 'python');
      expect(suggestion).toBeNull();
    });

    it('should return null for tool in wrong language', () => {
      // pytest only has Python install command, not Node.js
      const suggestion = resolver.getSuggestedInstallCommand('pytest', 'nodejs');
      expect(suggestion).toBeNull();
    });
  });
  ...
  describe('getDetectionSummary', () => {
    ...
    it('should generate summary without package manager', async () => {
      mockLanguageDetector.getMakefileTargets.mockResolvedValue(['lint', 'test']);
