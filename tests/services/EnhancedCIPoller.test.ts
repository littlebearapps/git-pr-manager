...
describe("EnhancedCIPoller", () => {
  ...
  beforeEach(() => {
    ...
    (ErrorClassifier as jest.Mock).mockImplementation(
      () => mockErrorClassifier,
    );
    (SuggestionEngine as jest.Mock).mockImplementation(
      () => mockSuggestionEngine,
    );
  ...
  describe("getDetailedCheckStatus", () => {
    it("should fetch and parse check status for PR", async () => {
      ...
      mockOctokitMethods.checks.listForRef.mockResolvedValue(
        mockCheckRuns as any,
      );
      mockOctokitMethods.repos.getCombinedStatusForRef.mockResolvedValue(
        mockCommitStatus as any,
      );
  ...
  describe("getCheckAnnotations", () => {
    it("should fetch annotations for check run", async () => {
      ...
      mockOctokitMethods.checks.listAnnotations.mockResolvedValue(
        mockAnnotations as any,
      );
  ...
  describe("calculateNextInterval", () => {
    ...
    it("should reduce interval for fast check duration (adaptive)", () => {
      ...
      const interval = (poller as any).calculateNextInterval(
        10000,
        strategy,
        5000,
      );
    ...
    describe("waitForChecks - no checks scenario", () => {
      it("should detect no CI checks configured", async () => {
        ...
        expect(infoSpy).toHaveBeenCalledWith(
          expect.stringContaining("Skipping CI check wait"),
        );
      });
  ...
  describe("isRetryable", () => {
    it("should return true when failure matches retry pattern", () => {
      const status = {
        failureDetails: [{ summary: "Connection timeout error" }],
      } as any;

      const result = (poller as any).isRetryable(status, [
        "timeout",
        "network",
      ]);
    ...
    it("should return false when failure does not match pattern", () => {
      const status = {
        failureDetails: [{ summary: "Type error in auth.ts" }],
      } as any;
    ...
    it("should be case insensitive", () => {
      const status = {
        failureDetails: [{ summary: "NETWORK ERROR" }],
      } as any;
  ...
  describe("hasCriticalFailure", () => {
    it("should return true for test failures", () => {
      const status = {
        failureDetails: [{ errorType: ErrorType.TEST_FAILURE }],
      } as any;
    ...
    it("should return true for build errors", () => {
      const status = {
        failureDetails: [{ errorType: ErrorType.BUILD_ERROR }],
      } as any;
    ...
    it("should return true for security issues", () => {
      const status = {
        failureDetails: [{ errorType: ErrorType.SECURITY_ISSUE }],
      } as any;
    ...
    it("should return false for non-critical failures", () => {
      const status = {
        failureDetails: [{ errorType: ErrorType.LINTING_ERROR }],
      } as any;
  ...
  describe("getNewFailures", () => {
    ...
    it("should return new failures not in previous status", () => {
      const prev = {
        failureDetails: [{ checkName: "test1" }],
      } as any;

      const current = {
        failureDetails: [{ checkName: "test1" }, { checkName: "test2" }],
      } as any;
  ...
  describe("getNewPasses", () => {
    ...
    it("should return checks that passed (no longer in failure list)", () => {
      ...
      const current = {
        failureDetails: [{ checkName: "test2" }],
      } as any;
